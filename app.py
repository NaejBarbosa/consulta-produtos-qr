...
def load_data():
    ...
    df['texto_busca_norm'] = df['texto_busca'].apply(normalizar_texto)   # campo normalizado
    return df
...
def buscar_produtos(termo, limite=5):
    ...
    resultados = process.extract(termo_norm, textos_norm, scorer=fuzz.token_set_ratio, limit=limite)
